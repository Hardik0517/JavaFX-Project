...
public class ViewBookingController implements Initializable {
    ...
    private void handleButtonActionDelete(ActionEvent event) throws IOException {
        if(delflight.getText().matches("")){
            l3.setText("Enter FlightId to cancel");
    }
        else{
            try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/dreamtravel", "root", "");
            Statement s = con.createStatement();
            PreparedStatement statement = con.prepareStatement("select * from booked where username='"+userna+"' and fid='"+delflight.getText()+"'");
            ResultSet rs = statement.executeQuery();
            if(rs.next()==false){
                l3.setText("Enter Valid FlightId");
            }
            else{
                String sq = "Delete from booked where username='"+userna+"' and fid='"+delflight.getText()+"'";
                 s.executeUpdate(sq);
